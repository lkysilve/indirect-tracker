...
// @version      8.16.0
// @description  Body-level SCC Tracker button placed in empty space directly before the real Replan button. Uses SCC unlock -> lock -> approve flow.
...
(function () {
  ...
  /******************************************************************
   * LCY2 SCC Approve Injector v8.16 hard-visible launcher + corrected role aliases
   *
   ...
   * another SCC role, v8.16 follows SCC's real flow: unlock old role, lock tracker role, then approve.
   *
   * Workflow:
  ...
  const ROLE_IGNORE_FOR_SCC = new Set(["IT WS"]);
  ...
  function toast(msg, bad = false) {
    ...
    if (!el) {
      ...
      el.style.cssText = `
        ...
        background:#111827;color:white;padding:10px 13px;border-radius:12px;
        font-weight:800;font-size:13px;box-shadow:0 12px 32px rgba(0,0,0,.24);
        max-width:620px;
      `;
  ...
  function getIdCache() {
    try { return JSON.parse(localStorage.getItem(ID_CACHE_KEY) || "{}"); }
    catch { return {}; }
  }
  ...
  function getEmployeeId(p) {
    ...
    for (const [key, value] of Object.entries(p)) {
      if (/employee|associate|badge/i.test(key)) {
        const id = isNumericEmployeeId(value);
        if (id) return id;
      }
    }

    // Do not use p.id because tracker rows often store UUIDs there.
    return "";
  }
  ...
  function parseTrackerPlan(data, date) {
    ...
    const plan =
      book?.plans?.[date] ||
      data?.plannerState?.plans?.[date] ||
      data?.plannerState ||
      data?.shiftPlanner?.plans?.[date] ||
      {};
    ...
    for (const role of roles) {
      ...
      const logins = assigned.map(item => {
        const login = typeof item === "string" ? item.toLowerCase() : clean(item.login).toLowerCase();
        const idFromItem = typeof item === "object" ? getEmployeeId(item) : "";
        return {
          login,
          note: typeof item === "object" ? clean(item.note) : "",
...
` +
  ...
  function renderPlan() {
    ...
    box.innerHTML = `
      <div class="lcy2-total">
        <b>${total}</b> planned
        <b>${ready}</b> IDs ready
        <b class="${missing ? "bad-pill" : ""}">${missing}</b> missing
      </div>
      ${lastInterceptSummary ? `<div class="lcy2-note">${escapeHtml(lastInterceptSummary)}</div>` : ""}
      <details class="lcy2-alias-details">
        <summary>Role matching used by SCC</summary>
        <div class="lcy2-alias-list">${roleAliasHelpHtml()}</div>
      </details>
  ...
  function applyFallbackLauncherStyle(btn) {
    ...
    // or SCC redraws its controls in a way our CSS cannot dock into.
    ...
    btn.style.setProperty("min-width", "76px", "important");
    btn.style.setProperty("padding", "0 10px", "important");
    btn.style.setProperty("border-radius", "4px", "important");
    btn.style.setProperty("border", "1px solid #14532d", "important");
    btn.style.setProperty("background", "#16a34a", "important");
    btn.style.setProperty("color", "#fff", "important");
    btn.style.setProperty("font", "700 11px Arial,sans-serif", "important");
    btn.style.setProperty("cursor", "pointer", "important");
    btn.style.setProperty("box-shadow", "0 4px 14px rgba(15,23,42,.18)", "important");
  ...
  function positionLauncherButton() {
    ...
    // v8.16: only anchor beside the REAL top-right Replan/Review button.
    // This prevents accidental anchoring near "Discard Changes" or other SCC buttons.
    ...
    if (target) {
      ...
      // Put Tracker directly in the empty gap to the LEFT of Replan/Review.
      // This keeps it close to the SCC buttons, but does not cover Review.
      let left = r.left - bw - gap;
      let top = r.top + ((r.height || 24) - bh) / 2;

      // Clamp safely inside viewport, but never jump all the way to the left.
  ...
  function positionDockedPanel() {
    ...
    const width = Math.min(440, Math.max(360, window.innerWidth - 24));
  ...
  function toggleDockedPanel() {
    ...
    if (!panel) {
      try { buildPanel(); } catch (e) { console.error("[LCY2 SCC Injector v8.16] buildPanel failed", e); }
      panel = document.getElementById("lcy2ApproveInjectorPanel");
    }
    if (!panel) {
      toast("Tracker panel failed to build. Check console for v8.16 error.", true);
      return;
    }
  ...
  function buildPanel() {
    ...
    panel.innerHTML = `
      <div class="lcy2-head">
        <b>Indirect Tracker → SCC v8.16</b>
      ...
      <div class="lcy2-body">
        ...
        <div class="lcy2-warning">
          Clean mode: <b>Load plan → Fetch IDs if needed → Live Direct Approve</b>.
          It uses SCC's real flow: <b>unlock</b> old role, <b>lock</b> tracker role, then <b>approve</b>.
        </div>

        <details class="lcy2-advanced">
          <summary>Advanced settings</summary>
          <label>Tracker data.json URL</label>
          <input id="lcy2InjectorDataUrl" value="${escapeHtml(s.dataUrl)}" />

          <div class="lcy2-modebox">
            <label class="inline">
              <input type="radio" name="lcy2InjectorMode" value="merge" ${s.mode !== "replace" ? "checked" : ""}>
              Merge planned roles into SCC
            </label>
            <label class="inline">
              <input type="radio" name="lcy2InjectorMode" value="replace" ${s.mode === "replace" ? "checked" : ""}>
              Replace SCC indirect roles with tracker plan
            </label>
          </div>
    ...
    style.textContent = `
      ...
      #lcy2SccLauncher:hover{background:#15803d !important}
      ...
      #lcy2ApproveInjectorPanel{
        display:none;position:fixed;right:12px;top:82px;width:440px;max-height:82vh;z-index:99999998;
        background:#fff;border:1px solid #cbd5e1;border-radius:12px;
        box-shadow:0 18px 50px rgba(15,23,42,.28);overflow:hidden;font-family:Arial,sans-serif;
      }
      #lcy2ApproveInjectorPanel.open{display:block}
      #lcy2ApproveInjectorPanel .lcy2-head{
        background:#111827;color:#fff;padding:10px 12px;display:flex;justify-content:space-between;align-items:center;
      }
      #lcy2ApproveInjectorPanel .lcy2-head button{
        background:#334155;color:#fff;border:0;border-radius:8px;font-weight:900;padding:4px 9px;cursor:pointer;
      }
      #lcy2ApproveInjectorPanel .lcy2-body{padding:12px;overflow:auto;max-height:calc(86vh - 44px)}
      #lcy2ApproveInjectorPanel label{display:block;font-size:12px;font-weight:900;margin:7px 0 4px;color:#334155}
      #lcy2ApproveInjectorPanel label.inline{display:flex;align-items:center;gap:7px;margin:6px 0;font-size:12px}
      #lcy2ApproveInjectorPanel input[type="text"],
      #lcy2ApproveInjectorPanel input[type="date"],
      #lcy2ApproveInjectorPanel input:not([type]){
        width:100%;box-sizing:border-box;border:1px solid #cbd5e1;border-radius:10px;padding:8px;font-size:13px;
      }
      #lcy2ApproveInjectorPanel button{
        border:1px solid #cbd5e1;background:#fff;border-radius:10px;padding:7px 9px;font-weight:900;cursor:pointer;
      }
      #lcy2ApproveInjectorPanel button:hover{background:#eff6ff}
      #lcy2ApproveInjectorPanel #lcy2InjectorLoad{background:#dcfce7;color:#166534;border-color:#86efac;margin-top:22px}
      #lcy2ApproveInjectorPanel #lcy2LiveDirectApprove{background:#16a34a;color:#fff;border-color:#15803d}
      #lcy2ApproveInjectorPanel .lcy2-row{display:grid;grid-template-columns:1fr 118px;gap:8px;align-items:end}
      #lcy2ApproveInjectorPanel .lcy2-modebox{background:#f8fafc;border:1px dashed #cbd5e1;border-radius:12px;padding:8px;margin:8px 0}
      #lcy2ApproveInjectorPanel .lcy2-warning{font-size:12px;color:#713f12;background:#fffbeb;border:1px solid #fde68a;padding:8px;border-radius:12px;margin:10px 0}
      #lcy2ApproveInjectorPanel .lcy2-note{font-size:12px;color:#475569;background:#f8fafc;border:1px dashed #cbd5e1;padding:8px;border-radius:12px;margin:8px 0}
      #lcy2ApproveInjectorPanel .lcy2-status{font-size:12px;color:#166534;background:#ecfdf5;border:1px solid #86efac;padding:7px;border-radius:10px;margin-top:9px;font-weight:800}
      #lcy2ApproveInjectorPanel .lcy2-status.bad{color:#991b1b;background:#fee2e2;border-color:#fecaca}
      #lcy2ApproveInjectorPanel .lcy2-buttons{display:flex;gap:6px;flex-wrap:wrap;margin:10px 0}
      #lcy2ApproveInjectorPanel .lcy2-armed{background:#fee2e2;color:#991b1b;border:1px solid #fecaca;border-radius:12px;padding:8px;margin:8px 0;font-weight:900}
      #lcy2ApproveInjectorPanel .lcy2-safe{background:#f8fafc;color:#475569;border:1px solid #e2e8f0;border-radius:12px;padding:8px;margin:8px 0;font-weight:900}
      #lcy2ApproveInjectorPanel .lcy2-total{display:flex;gap:8px;align-items:center;flex-wrap:wrap;background:#f8fafc;border:1px solid #e2e8f0;border-radius:12px;padding:8px;margin:10px 0}
      #lcy2ApproveInjectorPanel .lcy2-total b{background:#111827;color:#fff;border-radius:999px;padding:4px 8px}
      #lcy2ApproveInjectorPanel .lcy2-total b.bad-pill{background:#991b1b}
      #lcy2ApproveInjectorPanel .lcy2-role{border:1px solid #e2e8f0;border-radius:12px;margin:8px 0;padding:8px;background:#fbfdff}
      #lcy2ApproveInjectorPanel .lcy2-role-head{display:flex;justify-content:space-between;gap:8px;align-items:center}
      #lcy2ApproveInjectorPanel .lcy2-role-head span{color:#dc2626}
      #lcy2ApproveInjectorPanel .lcy2-role-head em{font-size:11px;color:#475569;font-style:normal;font-weight:900;background:#e2e8f0;border-radius:999px;padding:3px 7px}
      #lcy2ApproveInjectorPanel .lcy2-mini{font-size:12px;color:#64748b;margin-top:4px;font-weight:800}
      #lcy2ApproveInjectorPanel .lcy2-login-list{margin-top:7px;display:grid;gap:4px}
      #lcy2ApproveInjectorPanel .lcy2-login-row{display:flex;justify-content:space-between;gap:8px;font-size:12px;border-bottom:1px solid #e2e8f0;padding:4px 0}
      #lcy2ApproveInjectorPanel .lcy2-login-row.missing{color:#991b1b}
      #lcy2ApproveInjectorPanel .lcy2-login-row code{font-size:11px;background:#e2e8f0;border-radius:7px;padding:2px 5px;color:#334155}
      #lcy2ApproveInjectorPanel .lcy2-login-row.missing code{background:#fee2e2;color:#991b1b}
      #lcy2ApproveInjectorPanel details summary{cursor:pointer;font-weight:900;color:#334155;font-size:12px;margin-top:7px}
      #lcy2ApproveInjectorPanel .lcy2-advanced{background:#f8fafc;border:1px solid #e2e8f0;border-radius:12px;padding:8px;margin:8px 0}
      #lcy2ApproveInjectorPanel .lcy2-advanced summary,
      #lcy2ApproveInjectorPanel .lcy2-alias-details summary{cursor:pointer;font-weight:900;color:#334155;font-size:12px}
      #lcy2ApproveInjectorPanel .lcy2-alias-list{display:flex;gap:5px;flex-wrap:wrap;margin-top:7px}
      #lcy2ApproveInjectorPanel .lcy2-alias-chip{font-size:11px;font-weight:900;color:#334155;background:#e0f2fe;border:1px solid #bae6fd;border-radius:999px;padding:3px 7px}
      #lcy2ApproveInjectorPanel .lcy2-alias-details{background:#f8fafc;border:1px solid #e2e8f0;border-radius:12px;padding:8px;margin:8px 0}
      #lcy2ApproveInjectorPanel .lcy2-empty{font-weight:900;color:#64748b;text-align:center;padding:16px}
      #lcy2ApproveInjectorPanel .lcy2-body{display:block}
    `;
  ...
  installApproveHook();
  setInterval(refreshTemplateStatus, 2500);
  setInterval(() => {
    try {
      mountLauncherButton();
      if (panelOpen) positionDockedPanel();
    } catch (e) {
      console.warn("[LCY2 SCC Injector v8.16] launcher interval failed", e);
    }
  }, 800);
  window.addEventListener("resize", () => { mountLauncherButton(); if (panelOpen) positionDockedPanel(); });
  window.addEventListener("scroll", () => { mountLauncherButton(); if (panelOpen) positionDockedPanel(); }, true);
  ...
  function boot() {
    ...
    console.info("[LCY2 SCC Injector v8.16] booting hard-visible Tracker launcher");

    try {
      buildPanel();
    } catch (e) {
      console.error("[LCY2 SCC Injector v8.16] buildPanel failed, continuing with launcher fallback", e);
    }

    try {
      mountLauncherButton();
    } catch (e) {
      console.error("[LCY2 SCC Injector v8.16] mountLauncherButton failed, forcing fallback", e);
    ...
    // SCC is React-heavy and can redraw the action buttons after our script starts.
    // Keep watching and re-dock the Tracker button whenever the toolbar changes.
    const obs = new MutationObserver(() => {
      try {
        mountLauncherButton();
        if (panelOpen) positionDockedPanel();
      } catch (e) {
        console.warn("[LCY2 SCC Injector v8.16] redock failed", e);
      }
    });
  ...
  if (document.readyState === "loading") {
    ...
    // Extra safety: some SCC/Tampermonkey timing combinations miss the normal path.
